...
def text_handler(text):
    ...
    for s in sentences:
        ...
        s += delimiters[0]
        if len(s)>100:
            commaspliter=s.split(delimiters[1])
            for c in commaspliter:
                if c.strip() == "": continue
                c += delimiters[1]
                if len(c)>100:
                    pointspliter=c.split(delimiters[2])
                    for p in pointspliter:
                        if p.strip()=="":continue
                        p+= delimiters[2]
                        result.append(p)
                result.append(c)
...
def Generate_Audio(text,id):
        ...
        sentences=text_handler(text)
        wavs = []
        s_prev = None
        path= "Demo/reference_audio/Hamza.wav"
        ref_s = compute_style(path)
        lengths=0
        for s in sentences:
            if s.strip() == "": continue
            lengths+=len(s)
            print("length :",len(s),"with ",len(text)-lengths,"left \n")
            wav, s_prev = LFinference(s,
                                s_prev,
                                ref_s,
                                alpha = 0.3,
                                beta = 0.7,  # make it more suitable for the text
                                t = 0.8,
                                diffusion_steps=5, embedding_scale=1.5)
            wavs.append(wav)
            
        samplerate = 22000
        bitdepth = 16  # Adjust for desired bit depth (8, 16, 24)
        ...
        sf.write(
                ...
                np.concatenate(wavs),
                samplerate,
                subtype=f"PCM_{bitdepth}",  # Specify bit depth in subtype
            )
